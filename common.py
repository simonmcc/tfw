...
def tag_exists(SHA=None):
    if SHA is None:
        raise ValueError('ERROR: there was a problem looking up AMI by sha')

    try:
        # EMPTY=$(aws ec2 describe-images --filters Name=tag:SHA,Values=$SHA --query 'Images[*]')
        ec2 = boto3.resource('ec2')
        ami_filter = [{'Name': 'tag:SHA', 'Values': [SHA]}]
...
def get_git_branch():
    ...
    git_branch = git('rev-parse', '--abbrev-ref', 'HEAD').rstrip()
    ...
    if git_branch == 'HEAD':
        ...
        git_branch = git('branch', '--remote', '--verbose', '--no-abbrev', '--contains').rstrip()

    return git_branch


def map_branch_to_workspace(branch):
    print branch
    if branch == 'master':
        return 'default'
    else:
